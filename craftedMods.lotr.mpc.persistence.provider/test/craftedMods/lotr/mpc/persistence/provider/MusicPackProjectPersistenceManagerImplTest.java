...
public class MusicPackProjectPersistenceManagerImplTest extends EasyMockSupport
{
    ...
    public void testLoadMusicPackProjects () throws IOException
    {
        ...
        EasyMock.expect (mockFileManager.getPathsInDirectory (projectsDir))
            .andReturn (Arrays.stream (new Path[]
            {projectPath1})).once ();
        ...
        mockCompatibilityManager.applyPostLoadFixes (EasyMock.eq (projectPath1),
            EasyMock.capture (postLoadProjectCapture),
            EasyMock.eq ("0.1.0"));
    ...
    public void testLoadMusicPackProjectProjectSpecificLoadingError () throws IOException
    {
        ...
        EasyMock.expect (mockFileManager.getPathAndCreateDir (projectsDir.toString ())).andStubReturn (projectsDir);
        EasyMock.expect (mockFileManager.getPathsInDirectory (projectsDir))
            .andStubReturn (Arrays.stream (new Path[]
            {projectPath1}));
        EasyMock.expect (mockFileManager.isDirectory (EasyMock.anyObject ())).andStubReturn (true);
        EasyMock.expect (mockFileManager.exists (EasyMock.anyObject ())).andStubReturn (true);
    ...
    public void testLoadMusicPackProjectNonSemanticVersion () throws IOException
    {
        ...
        mockCompatibilityManager.applyPostLoadFixes (projectPath1, mockMusicPackProject1,
            "Music Pack Creator 10.12.14");
        EasyMock.expectLastCall ().once ();
    ...
    @Test
    public void testSaveNonExistingMusicPackProjectWithoutBackup () throws IOException
    {
        Path projectPath = projectsDir.resolve ("proj_3");

        EasyMock.expect (mockFileManager.exists (EasyMock.anyObject ())).andReturn (true).times (3).andReturn (false)
            .once ();
        ...
        saveExistingMusicPackProject (project, projectPath, false, false, false);
    ...
    @Test
    public void testSaveNonExistingMusicPackProjectWithBackup () throws IOException
    {
        Path projectPath = projectsDir.resolve ("proj_3");
        ...
        saveExistingMusicPackProject (project, projectPath, false, false, true);
    ...
    @Test
    public void testSaveExistingMusicPackProjectWithoutBackup () throws IOException
    {
        saveExistingMusicPackProject (createMockMusicPackProject ("0.1.0"), projectsDir.resolve ("project1"), true,
            false, false);
    }

    @Test
    public void testSaveExistingMusicPackProjectWithBackup () throws IOException
    {
        saveExistingMusicPackProject (createMockMusicPackProject ("0.1.0"), projectsDir.resolve ("project1"), true,
            false, true);
    }
    ...
    private void saveExistingMusicPackProject (MusicPackProject project, Path projectPath, boolean inject,
        boolean exception, boolean testBackup) throws IOException
    {
        EasyMock.expect (mockFileManager.getPathAndCreateDir (projectsDir.toString ())).andStubReturn (projectsDir);
        ...
        EasyMock
            .expect (mockFileManager.exists (projectFilePath)).andReturn (testBackup);

        if (testBackup)
        {
            mockFileManager.rename (projectFilePath, MusicPackProjectPersistenceManagerImpl.OLD_PROJECT_FILE_NAME);
            EasyMock.expectLastCall ().once ();
        }
